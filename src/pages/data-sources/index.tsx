...
import { getDataSources, deleteDataSourceById } from 'apiSdk/data-sources';
import { DataSourceInterface } from 'interfaces/data-source';

type ColumnType = ColumnDef<DataSourceInterface, unknown>;
...
export function DataSourceListPage(props: DataSourceListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DataSourceInterface>>(
    () => `/data-sources?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DataSourceInterface) => {
    if (hasAccess('data_source', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/data-sources/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/data-sources/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('data_source', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/data-sources/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('data_source', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="data_source" />
        </Flex>
        {hasAccess('data_source', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/data-sources/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
