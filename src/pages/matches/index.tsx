...
export function MatchListPage(props: MatchListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getMatches({
        relations: ['user_match_user_id1Touser', 'user_match_user_id2Touser', 'date_schedule.count'],
  ...
  const columns: ColumnType[] = [
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_match_user_id1Touser',
          header: 'User Match User Id 1 Touser',
          accessorKey: 'user_match_user_id1Touser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_match_user_id1Touser?.id}`}
            >
              {record.user_match_user_id1Touser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_match_user_id2Touser',
          header: 'User Match User Id 2 Touser',
          accessorKey: 'user_match_user_id2Touser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_match_user_id2Touser?.id}`}
            >
              {record.user_match_user_id2Touser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('date_schedule', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'date_schedule',
          header: 'Date Schedule',
          accessorKey: 'date_schedule',
          cell: ({ row: { original: record } }: any) => record?._count?.date_schedule,
        }
      : null,
