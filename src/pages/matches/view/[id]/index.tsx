...
import { DateScheduleListPage } from 'pages/date-schedules';
...
function MatchViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<MatchInterface>(
    ...
    () =>
      getMatchById(id, {
        relations: ['user_match_user_id1Touser', 'user_match_user_id2Touser'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Match User Id 1 Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_match_user_id1Touser?.id}`}>
                        {data?.user_match_user_id1Touser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Match User Id 2 Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_match_user_id2Touser?.id}`}>
                        {data?.user_match_user_id2Touser?.email}
                      </Link>
                    }
                  />
                )}
